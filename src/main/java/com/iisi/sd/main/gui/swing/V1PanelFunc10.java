...
public class V1PanelFunc10 extends JPanel {
	...
	public V1PanelFunc10() {
		...
		JLabel cation01Label = new JLabel("注意:XLDF002M,XLDF005M在使用工具產生時,要手動加入\"VISIBLE , VARCHAR(05) ,使否列印申請書及顯示在記事清單\"欄位");
		add(cation01Label, "2, 6, 13, 1");
		
		add(execButton, "2, 26, left, bottom");
	}
	 
	private  Pattern rldfPattern = Pattern.compile("6-4-RLDF[U|R|W|S|M|0]\\d{2}[T|M|\\d]\\.doc");
			...
			if(srcDir.getFile()!=null && destiDir.getFile()!=null){
				...
				for(File srcFileUnit: srcFile){
					if(rldfPattern.matcher(srcFileUnit.getName()).matches()){						
						...
						method.modiferOfXLDF(dataTable);
						dableList.add(method.modiferOfXLDF(dataTable));
					}
				}
				...
				Map<String, String> scriptFinalDataMap = service2.generateFinalScript(dableList);
				 try {
	                ...
	                JOptionPane.showMessageDialog(V1PanelFunc10.this, message, "匯出結果", JOptionPane.INFORMATION_MESSAGE);
			}else{
				JOptionPane.showMessageDialog(V1PanelFunc10.this, "請必須指定目錄.........", "操作錯誤", JOptionPane.ERROR_MESSAGE);
			}
	...
	private void handleExceptionErrors(Exception e1) {
		...
		JOptionPane.showMessageDialog(V1PanelFunc10.this, sbf.toString(), "操作錯誤", JOptionPane.ERROR_MESSAGE);
	}
