...
public abstract class AbstractComareTableServiceImpl {
	...
	private Collection<String>[] compareNames(final Set<String> aSet, Set<String> bSet) {
        ...
        for (String aName : aSet) {            
            String name = StringUtils.trim(aName).replace(String.valueOf(new char[] { (char) 160 }), "");
    ...
    private DiscrepancyTable comparaUnitColumns(final DiscrepancyTableInterface scriptTableInfo,
        ...
        final  Set<String> sQLScripDataColumn = extractDiscrepancyColumnInterface(scriptTableInfoColumnList);
        final  Set<String> wordDocDataColumn = extractDiscrepancyColumnInterface(dataTableColumnInfos);

        final  Collection<String>[] initData = compareNames(sQLScripDataColumn, wordDocDataColumn);
    ...
    private <T extends DiscrepancyColumnInterface> DiscrepancyTable compareUnitColumnAttributes(
        ...
        for (String columnName : sameColumns) {
        	T scriptColumn = scriptTableMap.get(columnName);
        	T dataColumnInfo = dataTableMap.get(columnName);
        	Map<String, String[]> data =  columnAttributeComparisonLogic(scriptColumn, dataColumnInfo, new String[]{"format","primaryKey"});
        	for(String key : data.keySet()){
        		String[] value = data.get(key);
        		sameColumnFormatDiscrepancy.put(key, value);
        		if(value.length>0){
        			aDiscrepancyTable.setDiscrepancy(true);
        		}
        	}
        }
        
        aDiscrepancyTable.setSameColumnFormatDiscrepancy(sameColumnFormatDiscrepancy);
        return aDiscrepancyTable;
    }
    
    abstract<T extends DiscrepancyColumnInterface> Map<String, String[]>  columnAttributeComparisonLogic(T scriptColumn, T dataColumnInfo,String[] attributes) ;
